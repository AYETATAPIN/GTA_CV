    ...
    for car_label in cars_labels:
        ...
        if plate_inside_car(plate_coords, car_coords) == True:
            if min_distance_value == -1:
                min_distance_value = calc_distance(plate_coords, car_coords)
                corresponding_car_image = car_img
            else:
                temp = calc_distance(plate_coords, car_coords)
                if temp < min_distance_value:
                    min_distance_value = temp
                    corresponding_car_image = car_img
...
def start_footage_processing(camera_url):
    ...
    while True:
        ...
        for plate_image in plates_images:
            ...
            extracted_text = plate_response_body["Detected text"].lower()
