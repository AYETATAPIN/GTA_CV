...
def save_one_box(xyxy, im, file=Path("im.jpg"), gain=1.02, pad=10, square=False, BGR=False, save=True):
    ...
    """
    xyxy_original = (torch.tensor(xyxy).long().view(1, 4)).view(-1).tolist()
    ...
    if save:
        ...
        image_file = str(increment_path(file).with_suffix(".jpg"))
        crop_coords_file_name = str(increment_path(file).with_suffix(".txt"))
        with open(crop_coords_file_name, "w") as crop_coords_file:
            crop_coords_file.write(f"{xyxy_original[0]} {xyxy_original[1]} {xyxy_original[2]} {xyxy_original[3]}") # x1 y1 x2 y2
        # cv2.imwrite(f, crop)  # save BGR, https://github.com/ultralytics/yolov5/issues/7007 chroma subsampling issue
        Image.fromarray(crop[..., ::-1]).save(image_file, quality=95, subsampling=0)  # save RGB
    return crop
